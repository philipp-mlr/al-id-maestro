...
func NewConfig(db *sqlx.DB) (*config.Config, error) {
	c := config.Config{RemoteConfiguration: []config.RemoteConfiguration{}}
	...
	err = database.DeleteDiscoveredObjectsNotInRepositories(db, repositories)
	if err != nil {
		return nil, err
	}
