...
func InitRepos(config *config.Config) error {
	cloneInMemoryEnv := os.Getenv("CLONE_IN_MEMORY")
	v := strings.Contains(strings.ToLower(cloneInMemoryEnv), "true")
...
func GetRepositories(config *config.Config, cloneInMemory bool) error {
	for i, c := range config.RemoteConfiguration {
		r, err := cloneRepository(c, cloneInMemory)
...
func Walk(config *config.RemoteConfiguration, f func(f *object.File) error) error {
	// Walk the tree
...
func GetRemoteBranches(config *config.RemoteConfiguration) ([]model.Branch, error) {
...
func cloneRepository(config config.RemoteConfiguration, cloneInMemory bool) (*git.Repository, error) {
	if cloneInMemory {
...
func openOrCloneRepositoryOnDisk(config config.RemoteConfiguration) (*git.Repository, error) {
...
func cloneRepositoryInMemory(config config.RemoteConfiguration) (*git.Repository, error) {
	fs := memfs.New()
