...
func scanRepository(config *config.RemoteConfiguration, db *sqlx.DB) error {
	branches, err := git.GetRemoteBranches(config)
	if err != nil {
		return err
	}

	database.DeleteDiscoveredObjectNotInBranches(db, branches, config.RepositoryName)

	for i, branch := range branches {
		log.Printf("%d/%d | syncing branch %s", i+1, len(branches), branch.Name)

		commitID := database.GetLastCommitID(db, branch.Name, config.RepositoryName)
		...
		err = git.Checkout(config.Git, config.AuthContext, branch.Name, config.RemoteName)
		if err != nil {
			log.Printf("Error durching checkout on branch %s %s", branch.Name, err)
			continue
		}

		err = git.Pull(config.Git, config.AuthContext, config.RemoteName, branch.Name)
		if err != nil {
...
func scanAndInsertRepositoryFiles(db *sqlx.DB, config *config.RemoteConfiguration, branch model.Branch) error {
	var found []model.DiscoveredObject
	var apps []model.AppJsonFile

	countALFiles := 0

	err := git.Walk(config, func(f *object.File) error {
		if !f.Mode.IsFile() {
			return nil
		}
	...
	if err = database.DeleteDiscoveredObjectsByBranchAndRepo(db, branch.Name, config.RepositoryName); err != nil {
		return err
	}
