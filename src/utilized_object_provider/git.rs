...
impl super::UtilizedObjectProvider for GitUtilizedObjectProvider {
    fn generate_id_tree(&self) -> ObjectTree {
        ...
        let repo = Self::open_repo(path, url).unwrap();
        ...
        for branch_result in branches {
            ...
            let re = Regex::new(r#"^(?P<type>\w+) (?P<id>\d{1,6}) "?(?P<name>[^"]*)?"?""#).unwrap();
            ...
            for entry in WalkDir::new(path).into_iter().filter_map(|e| e.ok()) {
                ...
                let object_type = &caps["type"];
                let id = &caps["id"];
                let name = &caps["name"];

                let Ok(real_object_type) = ObjectType::from_str(object_type) else {
                    println!("bad object");
                    continue;
                };

                println!("{} {} {}", object_type, id, name);
            }
...
impl GitUtilizedObjectProvider {
    fn open_repo(path: &Path, url: &str) -> Result<Repository, git2::Error> {
        let Ok(username) = env::var("GIT_USERNAME") else {
            panic!("username not set")
        };
        let Ok(password) = env::var("GIT_PASSWORD") else {
