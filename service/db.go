...
func insertClaimedObject(db *sqlx.DB, claimedObject model.Claimed) error {
	stmt := `
...
func SelectClaimedObjects(db *sqlx.DB, offset uint64) ([]model.Claimed, error) {
	stmt := `
		SELECT *
			FROM claimed
			LIMIT 50 OFFSET 50 * ?
...
func SelectDuplicates(db *sqlx.DB, offset uint64) ([]model.Found, error) {
	stmt := `
...
func SelectFound(db *sqlx.DB, offset uint64) ([]model.Found, error) {
	stmt := `
...
func selectClaimedNotInFound(db *sqlx.DB) ([]model.Claimed, error) {
	stmt := `
		SELECT  c.id,
				c."type",
				c.in_git,
...
// func getclaimedNotInGit(db *sqlx.DB) ([]model.Claimed, error) {
// 	stmt := `
// 		UPDATE claimed
// 			SET in_git = true
// 			WHERE in_git = false
...
// func updateClaimedInGit(db *sqlx.DB, id uint, objectType model.ObjectType, value bool) error {
// 	stmt := `
// 	UPDATE claimed
// 		SET in_git = ?
// 		WHERE id = ?
...
func updateClaimedExpired(db *sqlx.DB, id uint, objectType model.ObjectType, value bool) error {
	stmt := `
	UPDATE claimed
		SET expired = ?
		WHERE id = ?
...
func updateClaimedNotInGit(db *sqlx.DB) error {
	stmt := `
	UPDATE claimed
	SET in_git = false
	WHERE NOT EXISTS (
...
func updateClaimedInGit(db *sqlx.DB) error {
	stmt := `
	UPDATE claimed
	SET in_git = true
	WHERE EXISTS (
...
func selectDistinctFoundByType(db *sqlx.DB, objectType model.ObjectType) ([]model.Found, error) {
	stmt := `
...
func selectDistinctClaimedByType(db *sqlx.DB, objectType model.ObjectType) ([]model.Claimed, error) {
	stmt := `
	SELECT DISTINCT id, type
		FROM claimed
		WHERE type = ?
...
func deleteRemovedBranches(db *sqlx.DB, branches []model.Branch, repository string) (int64, error) {
	stmt := `
	DELETE FROM found
		WHERE branch NOT IN (?)
	`
...
func deleteMissingRepositories(db *sqlx.DB, repositories []string) error {
	stmt := `
	DELETE FROM found
